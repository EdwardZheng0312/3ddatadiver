...
def threedzslicings(filename, z_dir, location_slices, export_filename2, x_actual, y_actual, x_size, y_size):
    z_direction, Z_dir = zdirection.zdirection(filename, z_dir)
    phaseshift = (createpslist.createpslist(filename,x_size, y_size))[location_slices-1].flatten()
    ...
    im = ax.scatter(X, Y, Z, c=l, s=6, vmax=np.array(createpslist.createpslist(filename, x_size, y_size)).max(), vmin=np.array(create_pslist.create_pslist(filename, x_size, y_size)).min())
