...
class load_data(tk.Frame):
    ...
    def __init__(self, parent, controller):  # Define all the controller in the load_data window
        ...
        button3 = ttk.Button(self, text="2D Slicing Animation", command=lambda: controller.show_frame(slicing_animation))
        button3.pack(pady=5, padx=5)
...
class twoD_slicing(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        button6 = ttk.Button(self, text="2D Slicing Animation", command=lambda: controller.show_frame(slicing_animation))
...
class slicing_animation(tk.Frame):
    ...
    def slice(self, x_numslices, y_numslices, z_numslices): # get how many slices the user wants
        ...
        if y_num_slice in range(y_size):
            ...
            tkMessageBox.askretrycancel("Input Error", "Out of range, The expected range for y_num_slice is between 0 to " + str(len(y_size)) + ".")
        if z_num_slice in range(len(Z_dir)):
            pass
        else:
            tkMessageBox.askretrycancel("Input Error", "Out of range, The expected range for z_num_slice is between 0 to " + str(len(Z_dir)) + ".")
    ...
    def get_ani_range(self, z_ani_range): #x&y: user should choose from 0.5-2.0nm; z: user should choose from 1.0-4.0
        ...
        if (zanirange<Z_dir.max() or zanirange==Z_dir.max()) & (zanirange<Z_dir.min()):
            pass
        else:
            tkMessageBox.askretrycancel("Input Error", "Out of range, The expected range for z range is between " + str(Z_dir.min()) + " to " + str(Z_dir.max()) + ".")
        return zanirange

    def save_Z_animation(self, Z_dir, x_actual, y_actual, x_size, y_size, pslist, zanirange): #x_num_slice, y_num_slice, z_num_slice
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
        ax.set_xlim(left=init, right=x_actual)
        ax.set_ylim(bottom=init, top=y_actual)
        ...
        for add in range(z_num_slice): # z_num_slice is the number of Z slices
            ...
            c = Z_dir[(int(float(zanirange/Z_dir.max())*len(Z_dir)//z_num_slice) * add)] # get every page of Z_dir
            x, z, y = np.meshgrid(a, c, b)
            l = np.array(pslist)[(int(float(zanirange/Z_dir.max())*len(Z_dir)//z_num_slice) * add), :, :] # get every page of the pslist
        ...
        for add in np.arange(x_num_slice):
            ...
            c = Z_dir[: int(float(zanirange/Z_dir.max())*len(Z_dir))]
            x, z, y = np.meshgrid(a, c, b)
            l = np.array(pslist)[init:int(float(zanirange/Z_dir.max())*len(Z_dir)), int(x_size//x_num_slice) * add, init:y_size]
        ...
        im_ani.save('ZYX Slicing animation for ' + str(valu) + ' data.gif', writer='imagemagick', extra_args="convert", fps=300)
        return
