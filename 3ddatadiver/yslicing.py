...
def yslicing(filename, Z_direction, Y, x_actual, y_actual, x_size, y_size):
    ...
    for k in range(len(c)):
        for i in range(len(a)):
            A = (pd.DataFrame(createpslist.createpslist(filename, x_size, y_size)[0][k]).transpose().iloc[j])[i]
            psasas.append(A)
    l = psasas
