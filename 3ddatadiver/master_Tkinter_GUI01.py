...
class Force_Curve_plot(tk.Frame):
    ...
    def pixel_converter(self, location_slicesslices):
        """Convert from the real z to pixel"""
        global location_slices_pixel
        location_slices_pixel = int(float(location_slicesclices / round(float(np.array(Z_dirdir).max()),2)) * len(Z_dirdir)) + 1
    ...
    def plot_force(self, location_slices_pixel, numclick, x_actual, y_actual):
        ...
        Y, X = np.meshgrid(x, y)
        plt.scatter(X, Y, c=phaseshift, picker=6)
        ...
        def onpick3(event):
            ...
            if len(clicks) == numclick:
                ...
                x = np.linspace(init, np.shape(cc)[0] - 1, np.shape(cc)[0])
                ax = fig.add_subplot(111)

                def my_func(x, y):
                    plt.plot(x, aves_list[y], "--")
                    plt.plot(x, aves_list_all, color='black')

                [my_func(x, y) for y in np.arange(len(clicks))]
...
class animation_cool(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        tk.Frame.__init__(self, parent)
        tk.Frame.configure(self, background='#ffffff')
        label = ttk.Label(self, text="Step 6: 2D Slicing Animation", font=Huge_Font, background='#ffffff')
