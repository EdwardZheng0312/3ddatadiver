...
class Sea(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for F in (data_cleaning, load_data, threeD_plot, twoD_slicing, slicing_animation, tutorial, acknowledge):
...
class data_cleaning(tk.Frame):
    ...
    def get_source(self, source, export_filename, valu):
        ...
        return FFM, Zsnsr, export_filename0, threeD_array, Zsnsr_threeD_array, arraytocorr, indZ, \
    ...
    def export_cleaned_data(self, reduced_array_approach, reduced_array_retract, export_filename0):
        ...
        h5file_retract = export_filename0 + str("_retract_") + str(valu) + str(".h5")
    ...
    def __init__(self, parent, controller):
        ...
        label3 = ttk.Label(self, text="Select Target Dataset", font=Large_Font, background='#ffffff')
        ...
        button0 = ttk.Button(self, text="Load File",
                             command=lambda: (self.get_source(source, export_filename, valu)))
        button0.pack(pady=5, padx=5)

        button1 = ttk.Button(self, text="Export File as .h5",
...
class load_data(tk.Frame):
    ...
    def get_data(self, txtxactual, txtyactual):
        """The function to return the inputs to the GUI functions"""
        global x_actual
        global y_actual
    ...
    def __init__(self, parent, controller):  # Define all the controller in the load_data window
        ...
        button0 = ttk.Button(self, text="Apply Parameters",command=lambda: self.get_data(txtxactual, txtyactual))
        button0.pack(pady=5, padx=5)
        ...
        button3 = ttk.Button(self, text="3D Slicing Animation", command=lambda: controller.show_frame(slicing_animation))
        button3.pack(pady=5, padx=5)
...
class slicing_animation(tk.Frame):
    global event1
    ...
    def save_Z_animation(self, Z_dir, x_actual, y_actual, x_size, y_size, pslist): #x_num_slice, y_num_slice, z_num_slice
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
        ax.set_xlim(left=init, right=x_actual)
        ax.set_ylim(bottom=init, top=y_actual)
        ...
        plt.rcParams['animation.ffmpeg_path'] = 'D:\software\ImageMagick\ImageMagick-7.0.8-Q16\ffmpeg.exe'
        plt.rcParams["animation.convert_path"] = "D:\software\ImageMagick\ImageMagick-7.0.8-Q16\magick.exe"
    ...
    def __init__(self, parent, controller):
        ...
        button2 = ttk.Button(self, text="Save Animation", command=lambda: self.save_Z_animation(Z_dir, x_actual, y_actual, x_size, y_size, pslist))
