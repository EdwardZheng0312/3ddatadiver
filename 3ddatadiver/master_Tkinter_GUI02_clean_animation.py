...
class twoD_slicing(tk.Frame):
    ...
    def twoDZ_slicings(self, location_slices_pixel_z, export_filename2, x_actual, y_actual):
        ...
        root3.state('zoomed')
        fig4 = plt.figure(figsize=(9, 9), facecolor='white')
        plt.imshow(l, vmax=np.nanmax(np.array(self.create_pslist(Z_direction))), vmin=np.nanmin(np.array(self.create_pslist(Z_direction))))
        plt.axis([init, x_size - 1, init, y_size - 1])
        plt.xlabel('X', fontsize=12)
        plt.ylabel('Y', fontsize=12)
        plt.title('2D Z Slicing (Z=' + str(round(Z_dir[(location_slices_pixel_z) - 1], 4)) + 'nm) for the ' + str(valu) + ' of AFM data', fontsize=13)
        plt.colorbar()
        canvas3 = FigureCanvasTkAgg(fig4, master=root3)
        canvas3.draw()
        ...
        def getpoints():
            global numinp
            global x
            numinp = int(float(numberinput.get()))
            x = np.array(plt.ginput(numinp))
            print(np.where(X==round(x[0][0],2)),np.where(Y==round(x[0][1], 2)))

        def ave(x):
            global ave
            #input = np.asarray(x)
            #idx = (np.abs(input - 1)).argmin()
            #ave = np.mean(input[idx],axis=0)
            blank.insert(0, x)

        def findforce():
            fig5 = plt.figure(figsize=(5, 5), facecolor='white')
            plt.scatter(x[0])
            plt.xlabel('X(nm)', fontsize=12)
            plt.ylabel('Y(nm)', fontsize=12)
            plt.title('Force Curve from the Selected Points')
            canvas3 = FigureCanvasTkAgg(fig5, root3)
            canvas3.draw()
            canvas3.get_tk_widget().pack(side=tk.LEFT)
            return canvas3

        check = tk.Button(root3, text='Get Number', bg='white', command=lambda: getpoints())
        check.place(x=1150, y=315)
        button0 = tk.Button(root3, text='List the Points', bg='white', command=lambda:ave(x))
        button0.place(x=1150, y=350)
        blank = ttk.Entry(root3)
        blank.place(x=1000, y=350)
        button1 = tk.Button(root3, text='Find the Force Curve', bg='white', command=lambda: findforce())
        button1.place(x=1000, y=385)
...
class tutorial(ttk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        vid = tk.Button(self, text="Play Video", bg='white', command=source)
