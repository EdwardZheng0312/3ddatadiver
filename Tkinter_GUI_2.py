...
class data_cleaning(tk.Frame):
    ...
    def get_source(self, source, export_filename):
        ...
        Zsnsr_threeD_array = self.generatearray()[2]
        arraytocorr = self.correct_slope(Zsnsr_threeD_array, controller)[0]
        indZ = self.correct_slope(Zsnsr_threeD_array, controller)[1]
    ...
    def correct_slope(self, Zsnsr_threeD_array, Drive_threeD_array, controller):
        ...
        if controller == 'On':
            for i in range(1, xSIZE):
                Zdriftx[i] = np.mean(Zcorr[CROP : ySIZE - CROP, i])
                Ddriftx[i] = np.mean(Dcorr[CROP : ySIZE - CROP, i])
                xSI = np.arange(1, xSIZE + 1)
                DcorrLIN = np.polyfit(xSI, Ddriftx, 1)
                DdriftxL = xSI * DcorrLIN[1] + DcorrLIN[2]
                ZcorrLIN = np.polyfit(xSI, Zdriftx, 1)
                ZdriftxL = xSI * ZcorrLIN[1] + ZcorrLIN[2]
                Zcorr[CROP : ySIZE - CROP, : ] = Zcorr[CROP : ySIZE - CROP, : ] - ZdriftxL
                Dcorr[CROP : ySIZE - CROP, : ] = Dcorr[CROP : ySIZE - CROP, : ] - DdriftxL
        else:
                Zdriftx = np.zeros(1, xSIZE)
                Ddriftx = np.zeros(1, xSIZE)
    ...
    def export_cleaned_data(self, file, Phase_reduced_array, Amp_reduced_array, export_filename0):
        ...
        METAdata = str(list(file.attrs.values()))
        ...
        attrs_export = list(str('AmpInvOLS:'), AmpInvOLS, str('AmpDrive:'), AmpDrive, str('Qfactor:'), Qfactor, str('FreqDrive:'), FreqDrive, str('FreqRes:'), FreqRes,  str('Xnm:'), Xnm, str('Ynm:'), Ynm)

        new_h5file_g3.create_dataset('METAdata', data=METAdata, dtype='f4')
        new_h5file_g3.create_dataset('Attrs_info_input_HDF5', data=attrs_export, dtype='f4')
        return new_h5file
...
class load_data(tk.Frame):
    ...
    def get_data(self, txtxactual, txtyactual, Phase_reduced_array_retract, Phase_reduced_array_approach, Amp_reduced_array_approach, Amp_reduced_array_retract):
        """The function to return the inputs to the GUI functions"""
        global x_actual
        global y_actual
    ...
    def __init__(self, parent, controller):  # Define all the controller in the load_data window
        ...
        tk.Frame.__init__(self, parent)
        tk.Frame.configure(self, background='#ffffff')
        label = ttk.Label(self, text="Step 2: Input Dataset Information for Visualization", font=Huge_Font,
                          background='#ffffff')
        label.pack(pady=10, padx=10)

        label_2 = ttk.Label(self, text="X Sample Length (nm)", font="Small_Font", background='#ffffff')
        label_2.pack(pady=10, padx=10)
        txtxactual = ttk.Entry(self)
        txtxactual.pack()

        label_3 = ttk.Label(self, text="Y Sample Length (nm)", font="Small_Font", background='#ffffff')
        label_3.pack(pady=10, padx=10)
        txtyactual = ttk.Entry(self)
        txtyactual.pack()

        label4 = ttk.Label(self, text="Select the Objectives", font=Large_Font, background='#ffffff')
        label4.pack(padx=10, pady=10)
        lab = LabelFrame(self)
        lab.pack()
        listbox = Listbox(lab, exportselection=0)
        ...
        button0 = ttk.Button(self, text="Apply Parameters",command=lambda: self.get_data(txtxactual, txtyactual, Phase_reduced_array_retract, Phase_reduced_array_approach, Amp_reduced_array_retract, Amp_reduced_array_approach))
        button0.pack(pady=5, padx=5)
...
class twoD_slicing(tk.Frame):
    ...
    def twoDX_slicings(self, location_slices_pixel_x, export_filename2, x_actual, y_actual):
        ...
        a = np.linspace(init, x_actual, x_size)[
            location_slices_pixel_x]  # Define the certain x slice in the x space
    ...
    def twoDY_slicings(self, location_slices_pixel_y, export_filename2, x_actual, y_actual):
        ...
        fig = plt.figure(figsize=(9, 11), facecolor='white')
        ax = fig.add_subplot(111, projection='3d')
        im = ax.scatter(X, Y, Z, c=Bs.flatten(), s=6, vmax=np.array(self.create_pslist(Z_direction)).max(),
                        vmin=np.array(self.create_pslist(Z_direction)).min())
    ...
    def twoDZ_slicings(self, location_slices_pixel_z, export_filename2, x_actual, y_actual):
        ...
        fig = plt.figure(figsize=(9, 11), facecolor='white')
        ax = fig.add_subplot(111, projection='3d')
        im = ax.scatter(X, Y, Z, c=l.flatten(), s=6, vmax=np.array(self.create_pslist(Z_direction)).max(),
                        vmin=np.array(self.create_pslist(Z_direction)).min())
        #  Add and label colorbar
        cbar = plt.colorbar(im)
        cbar.set_label(str(valu))
    ...
    def __init__(self, parent, controller):
        ...
        label_1 = ttk.Label(self, text="Slices Location (nm)", font="Small_Font", background='#ffffff')
